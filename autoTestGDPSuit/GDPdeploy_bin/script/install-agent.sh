...
if test  $5 = '1'
	...
	if test $1 = 'x86'
		...
		ssh root@${array_node[0]}  "cd ${path_node[0]}  ;./${file_node[0]} -- --v --single-server --ip ${array_node[0]} --docker-images=docker-images-gdp-20240813.tar.bz2  >> ./install_x86_aio_admin_storage.log;echo $? >> ./install_x86_aio_admin_storage.log "
		#ssh root@${array_node[0]}  "cd ${path_node[1]};systemctl restart docker;./${file_node[1]} > ./install_x86_storage.log;echo $? >> ./install_x86_storage.log"
